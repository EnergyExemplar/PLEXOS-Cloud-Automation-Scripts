...
# ---------------- DOWNLOAD INPUTS ----------------
def download_inputs(local_mode=False):
    ...
    response = pxc.datahub.download(
        remote_glob_patterns=[
            "calibration/inputs/Netload_2027.xlsx"
        ],
        output_directory=str(OUTPUT_PATH),
        print_message=True
    )

    # Note: all_generators.xlsx was removed from the download list — the script
    # only requires Netload_2027.xlsx for net-load calculation.
...
def main() -> int:
    ...
    # ---------------- INIT ----------------
    netload_path = download_inputs(local_mode=args.local_mode)
    ...
    # TimeSeries/Bid Adders directory so --bidadder-filename behaves
    # consistently in both local and cloud modes.
    output_path = Path(simulation_path) / "TimeSeries" / "Bid Adders"
    ...
    if args.local_mode:
        ...
        # so calibration/inputs/Netload_2027.xlsx lands under OUTPUT_PATH.
        input_folder = OUTPUT_PATH / "calibration" / "inputs"
    ...
    netload_file = input_folder / "Netload_2027.xlsx"
    ...
    if not netload_file.exists():
        ...
        raise FileNotFoundError(f"Netload_2027.xlsx not found at {netload_file}")
    ...
    if timeseries_zip.exists() and not args.local_mode:
        ...
        # The entry inside the zip matches: TimeSeries/Bid Adders/<filename>
        zip_entry_name = f"TimeSeries/Bid Adders/{existing_file.name}"
