...
- Piecewise-linear markup interpolation from net-load normalization
- Monthly seasonal weight multipliers for fine-grained control
...
- Local mode for development/testing without DataHub access
...
- **Before this script:** Upload `Netload_2027.xlsx` to DataHub at path `calibration/inputs/Netload_2027.xlsx` — this path is hardcoded in the script (see [DataHub Prerequisites](#datahub-prerequisites))
...
| File | DataHub Path (sample — hardcoded in script) | Description |
|------|----------------------------------------------|-------------|
| `Netload_2027.xlsx` | `calibration/inputs/Netload_2027.xlsx` | Load, Solar, and Wind data (3 sheets). This path is hardcoded in the script — ensure the file is uploaded to exactly this location. |
...
| `--bidadder-filename` | Yes | — | Name of the existing bid adder file in `TimeSeries/Bid Adders/` (e.g. `Plexos_Markup_Adders_AllGenerators-2027.csv`). |
...
[
  {
    ...
    "Arguments": "python3 bid_adder_generation.py --bands \"0.0:0,0.6:0,0.8:125,0.9:625,1.0:1125\" --seasonal-weights \"1:0.50,2:0.55,3:1.30,4:1.50,5:1.57,6:1.30,7:1.40,8:1.47,9:1.10,10:1.15,11:1.10,12:1.05\" --counter 2 --bidadder-filename Plexos_Markup_Adders_AllGenerators-2027.csv",
