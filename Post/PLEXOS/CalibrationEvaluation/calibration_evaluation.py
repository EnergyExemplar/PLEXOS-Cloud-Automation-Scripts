...
def enqueue_next_simulation(next_counter, current_avg, threshold_value, simulation_file_path, margin):
    ...
    tasks_updated = {"generate_bid_adder": False, "evaluate_results": False}
    
    for task in payload["simulationOptions"]["simulationTasks"]:
        task_name = task["name"]
        
        if task_name == "generate_bid_adder":
            ...
            bidadder_filename = "Plexos_Markup_Adders_AllGenerators-2027.csv"  # fallback
            ...
            tasks_updated["generate_bid_adder"] = True
            print(f"[OK] Updated generate_bid_adder:", flush=True)
        ...
        elif task_name == "evaluate_results":
            new_args = (
            ...
            task["arguments"] = new_args
            tasks_updated["evaluate_results"] = True
            print(f"[OK] Updated evaluate_results:", flush=True)
            print(f"     Counter: {next_counter}", flush=True)
