...
class TestDownloadInputs:
    ...
    def test_local_mode_skips_download(self, capsys):
        ...
        MOD.download_inputs("calibration/inputs/Netload_2027.xlsx", local_mode=True)
...
class TestDownloadInputsSDK:
    ...
    def test_cloud_mode_calls_sdk(self, monkeypatch):
        ...
        with patch.object(MOD, "SDKBase") as mock_sdkbase:
            ...
            with patch.object(MOD, "CloudSDK", return_value=mock_sdk_instance):
                result_path = MOD.download_inputs("calibration/inputs/Netload_2027.xlsx", local_mode=False)
    ...
    def test_cloud_mode_uses_custom_netload_path(self, monkeypatch):
        """download_inputs should use the provided netload_file_path."""
        from unittest.mock import MagicMock, patch
        ...
        with patch.object(MOD, "SDKBase") as mock_sdkbase:
            ...
            mock_result = MagicMock()
            mock_result.Success = True
            mock_result.LocalFilePath = "/output/custom/path/MyNetload.xlsx"
            mock_data.DatahubResourceResults = [mock_result]
            mock_sdkbase.get_response_data.return_value = mock_data

            with patch.object(MOD, "CloudSDK", return_value=mock_sdk_instance):
                result_path = MOD.download_inputs("custom/path/MyNetload.xlsx", local_mode=False)

        call_kwargs = mock_sdk_instance.datahub.download.call_args
        assert "custom/path/MyNetload.xlsx" in call_kwargs.kwargs["remote_glob_patterns"]
