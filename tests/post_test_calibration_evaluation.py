...
class TestParserDefaults:
    ...
    def test_default_pre_task_name(self):
        args = MOD._make_parser().parse_args(["--simulation-file-path", "x.json"])
        assert args.pre_task_name == "generate_bid_adder"

    def test_default_post_task_name(self):
        args = MOD._make_parser().parse_args(["--simulation-file-path", "x.json"])
        assert args.post_task_name == "evaluate_results"

    def test_default_pre_script_name(self):
        args = MOD._make_parser().parse_args(["--simulation-file-path", "x.json"])
        assert args.pre_script_name == "bid_adder_generation.py"

    def test_default_post_script_name(self):
        args = MOD._make_parser().parse_args(["--simulation-file-path", "x.json"])
        assert args.post_script_name == "calibration_evaluation.py"

    def test_override_task_names(self):
        args = MOD._make_parser().parse_args([
            "--simulation-file-path", "x.json",
            "--pre-task-name", "my_pre_task",
            "--post-task-name", "my_post_task",
        ])
        assert args.pre_task_name == "my_pre_task"
        assert args.post_task_name == "my_post_task"

    def test_override_script_names(self):
        args = MOD._make_parser().parse_args([
            "--simulation-file-path", "x.json",
            "--pre-script-name", "my_gen.py",
            "--post-script-name", "my_eval.py",
        ])
        assert args.pre_script_name == "my_gen.py"
        assert args.post_script_name == "my_eval.py"
...
class TestEnqueuePayloadRewrite:
    ...
    def _make_payload(self):
        return {
            "simulationOptions": {
                "simulationTasks": [
                    {"name": "generate_bid_adder", "arguments": "python3 old_script.py --counter 0 --bidadder-filename test.csv --netload-file inputs/Netload.xlsx"},
...
class TestEnqueueSDKIntegration:
    ...
    def test_enqueue_calls_sdk_download_and_enqueue(self, monkeypatch, tmp_path):
        ...
        payload = {
            "simulationOptions": {
                "simulationTasks": [
                    {"name": "generate_bid_adder", "arguments": "python3 old.py --counter 0 --bidadder-filename MyAdders-2027.csv --netload-file calibration/inputs/Netload_2027.xlsx"},
...
class TestEnqueueSDKFallbackPath:
    ...
    def test_fallback_finds_payload_without_localfilepath(self, monkeypatch, tmp_path):
        ...
        payload = {
            "simulationOptions": {
                "simulationTasks": [
                    {"name": "generate_bid_adder", "arguments": "python3 old.py --counter 0 --bidadder-filename A.csv --netload-file data/netload.xlsx"},
                    {"name": "evaluate_results", "arguments": "python3 old.py --counter 0"},
                ]
            }
        }
